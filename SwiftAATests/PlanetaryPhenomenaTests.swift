...
class PlanetaryPhenomenaTests: XCTestCase {
    ...
    func testMercuryInferiorConjunction() {
        ...
        XCTAssertEqualWithAccuracy(mercury.inferiorConjunction().value, 2449297.644, accuracy: 0.001)
    }
    ...
    func testSaturnConjunction() {
        ...
        XCTAssertEqualWithAccuracy(saturn.conjunction().value, 2497437.903, accuracy: 0.001)
    }
    ...
    func testMercuryWesternElongation() {
        ...
        let mercury = Mercury(julianDay: jd)
        XCTAssertEqualWithAccuracy(mercury.westernElongation().value, 2449314.14, accuracy: 0.001)
