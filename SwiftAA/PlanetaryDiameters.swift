...
public extension PlanetaryDiameters {
    ...
    func equatorialSemiDiameter(usingOldValues: Bool = false) -> Degree {
        if (usingOldValues) {
            guard self.planet != .Pluto else {
                fatalError("There are no old value for Pluto.")
            }
    ...
    func polarSemiDiameter(usingOldValues: Bool = false) -> Degree {
        if (usingOldValues) {
